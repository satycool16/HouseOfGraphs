...
graph_id=$1
program_name="diameter"
input_format=$(jq .$program_name.input_format ./invariants.json | tr -d '"')

# executable
./pre_process.sh "$graph_id" "$program_name"
/opt/jdk-17/bin/java -cp ../diameter/src org.grinvin.hog.invariants.main.Diameter <  ../temp/graph_$graph_id.$input_format
